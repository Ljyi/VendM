...
namespace VendM.Core.Excel.Npoi
{
    //FileResult进行下载
    public class ExportExcelFileHelper<T>
    {
        /// <summary>
    ...
    /// 说明:导出Excel
    /// </summary>
    public abstract class ExcelBaseResult<T> 
    {
        #region 属性
        /// <summary>
        /// 数据实体
    ...
    //通过流
    public class ExportExcelHelper<T> : ExcelBaseResult<T> where T : new()
    {
        public ExportExcelHelper(IList<T> entity, string fileName, string title) : base(entity, fileName, title)
        {
        ...
        /// <param name="isCompress">是否压缩</param>
        /// <returns></returns>
        public static void ExportListToExcel_MvcResult(IList<T> list,ref string fileName, IList<string> headerList = null, IList<string> sortList = null, bool isCompress = false, int pagingSize = 65535)
        {
            //无数据情况
            if (list == null || list.Count == 0)
            {
            ...
            //如果是压缩，或者条数大于pagingSize，则也为压缩
            isCompress = isCompress || cnt > 1;
            var generateFileName = ExportListToExcel(list,ref fileName, headerList, sortList, isCompress, pagingSize, fileName);
        }

        private static string ExportListToExcel(IList<T> list, ref string fileName, IList<string> headerList = null, IList<string> sortList = null, bool isCompress = false, int pagingSize = 65535, string fileNameNew = null)
        {
            try
            {
                //服务器路径
